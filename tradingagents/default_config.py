...
DEFAULT_CONFIG = {
    ...
    # LLM settings（可通过环境变量覆盖，便于无头 / MCP 部署无需改代码切换供应商）
    ...
    # 环境变量 DEEP_THINK_PROVIDER_OVERRIDE=claude_agent_sdk 开启深思考节点走订阅。
    # None = 维持原行为（走 llm_provider）。
    "deep_think_provider_override": os.getenv("DEEP_THINK_PROVIDER_OVERRIDE") or None,
    # 环境变量 QUICK_THINK_PROVIDER_OVERRIDE=claude_agent_sdk 开启分析师节点走订阅。
    # 与上一项同时开启 = 全节点走订阅。None = 分析师仍走 llm_provider（维持原行为）。
    "quick_think_provider_override": os.getenv("QUICK_THINK_PROVIDER_OVERRIDE") or None,
    ...
    "agent_sdk_model": os.getenv("AGENT_SDK_MODEL", "opus"),
    # QUICK/分析师节点用的 Claude 模型。默认 sonnet 而非 opus——quick 节点数量多
    # （7 分析师 + 辩手），订阅是按额度限流的，全用 opus 很快会撞到上限。
    "agent_sdk_quick_model": os.getenv("AGENT_SDK_QUICK_MODEL", "sonnet"),
